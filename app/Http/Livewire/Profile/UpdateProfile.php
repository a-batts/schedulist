...
class UpdateProfile extends Component
{
    ...
    protected array $messages = [
        ...
        'state.email.unique' =>
            'Gadzooks, looks like that email is already in use. Your doppelgänger maybe?',
    ...
    public function save(): void
    {
        ...
        if (Auth::user()->phone != $this->state['phone']) {
            ...
            if ($lookupService->validate($this->state['phone'])) {
                ...
                if (
                    CarrierEmailHelper::getCarrierEmail(
                        $this->state['carrier']
                    ) == null
                ) {
                    throw ValidationException::withMessages([
                        'state.phone' =>
                            'The phone number you entered is not compatible.',
                    ]);
                }
                $this->formattedPhoneNumber = $this->getFormattedNumber(
                    $this->state['phone']
                );
    ...
    {
        try {
            $this->rateLimit(maxAttempts: 1, decaySeconds: 120);
            ...
            if (
                CarrierEmailHelper::getCarrierEmail($this->state['carrier']) ==
                null
            ) {
                throw ValidationException::withMessages([
            ...
            $body =
                'Hey there! Your verification code for Schedulist is ' .
                $code .
                '. If you didn\'t request one feel free to ignore this text.';
            $client->messages->create($number, [
                'body' => $body,
                'from' => env('TWILIO_PHONE_NUMBER', '+15715208808'),
            ]);
    ...
    {
        $user = Auth::user();
        $verificationCode = DB::table('two_factor_codes')
            ->where('user_id', Auth::id())
            ->first()->two_factor_code;

        if ($this->verificationCodeInput != $verificationCode) {
            throw ValidationException::withMessages([
                'verificationCodeInput' =>
                    'The verification code you inputted is incorrect.',
            ]);
        }
