...
class ClassScheduleHelper
{
    ...
    public function getNextClass(Carbon $dateTime): ?array
    {
        ...
        if (
            ...
            $dateTime->copy()->endOfDay() >
                Carbon::parse($activeSchedule->end_date)->endOfDay()
        ) {
            return null;
        }
    ...
    {
        return $this->schedules->first(function (ClassSchedule $item) use (
            $date
        ) {
            return $date >= Carbon::parse($item->start_date)->startOfDay() &&
                $date <= Carbon::parse($item->end_date)->endOfDay();
        });
    }
}
