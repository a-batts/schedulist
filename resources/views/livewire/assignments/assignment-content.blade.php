<div class="px-2 md:pt-10 md:px-24" x-data="{
    ...
    <div class="border-b border-gray-200 md:pb-4">
        <div class="flex py-5 mx-auto">
            <div class="flex items-center flex-grow">
                <button class="mdc-icon-button material-icons" aria-describedby="back-label" @click="window.history.back()"
                    wire:ignore>
                ...
                <div class="ml-8">
                    ...
                    <div class="mt-2 text-sm md:mt-6">
                        @if ($assignment->status->value == 1)
                            <p class="text-gray-500">Marked complete • Originally due on {{ $assignment->due_date }} at
                                {{ $assignment->due_time }}</p>
                        @else
            ...
            <div class="flex items-center space-x-3">
                <button class="mdc-icon-button material-icons" aria-describedby="edit-label"
                    @click="$dispatch('display-edit-menu')" wire:ignore>
                    <div class="mdc-icon-button__ripple"></div>
                    edit
                </button>
                <button class="mdc-icon-button material-icons" id="reminder-button"
                    aria-describedby="notifications-label" @if ($assignment->status->value == 1 || $assignment->is_late) disabled @endif
                    wire:ignore>
                    <div class="mdc-icon-button__ripple"></div>
    ...
    <div class="px-4 pt-5 pb-16 mt-4 sm:px-6 lg:px-8">
        ...
        <p class="mt-2 text-gray-600">Add additional details and instructions, as well as updates, to this assignment
        </p>
        <div class="flex w-full max-w-xl">
            <label class="mdc-text-field mdc-text-field--filled mdc-text-field--textarea flex-grow mt-7">
                <span class="mdc-text-field__ripple"></span>
                <span class="mdc-floating-label" id="note-contents">Add a note</span>
                <textarea class="mdc-text-field__input autosize" aria-label="Label" aria-labelledby="note-contents" rows="3"
                    x-model="noteContent" x-ref="noteBox"></textarea>
        ...
        <div class="w-full max-w-xl pt-6 space-y-6">
            @foreach ($this->notes as $note)
                <div class="mdc-card mdc-card--outlined w-full px-6 py-5">
                    <div class="flex gap-x-2">
                        ...
                        <button class="mdc-icon-button material-icons self-start -mt-2"
    ...
    <div class="fab-button">
        <button class="mdc-fab mdc-fab--extended mdc-button-ripple" aria-label="Add Assignment"
