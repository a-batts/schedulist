<div x-data="assignmentList()" @update-assignments.window="assignments = @this.assignments" wire:ignore>
        ...
        <div class="mb-4 basis-full md:basis-1/2">
            <label class="mdc-text-field mdc-text-field--filled mdc-text-field--with-leading-icon w-full">
                ...
                <i class="material-icons mdc-text-field__icon mdc-text-field__icon--leading" role="button"
                    tabindex="0">search</i>
                <input class="mdc-text-field__input" type="text" aria-labelledby="search-label" x-model="query">
            ...
            <div class="mdc-select mdc-select--filled w-full">
                ...
                <div class="mdc-select__menu mdc-menu mdc-menu-surface mdc-menu-surface--fullwidth">
                    <ul class="mdc-deprecated-list dark-theme-list" role="listbox">
                        <li class="mdc-deprecated-list-item @if ($class == -1) mdc-deprecated-list-item--selected @endif"
                            data-value="-1" role="option"
                            @if ($class == -1) aria-selected="true" @else aria-selected="false" @endif
                            @click="setFilterClass(-1, '')">
                        ...
                        @foreach ($classes as $each)
                            <li class="mdc-deprecated-list-item @if ($class == $each['id']) mdc-deprecated-list-item--selected @endif"
                                data-value="{{ $each['id'] }}" role="option"
                                @if ($class == $each['id']) aria-selected="true" @else aria-selected="false" @endif
                                @click="setFilterClass( {{ $each['id'] }}, '{{ $each['name'] }}')">
                                <span class="mdc-deprecated-list-item__ripple"></span>
                                <span class="mdc-deprecated-list-item__text">
                                    {{ $each['name'] }}
                                </span>
    ...
    <div class="pt-2 pb-6 border-b border-gray-200">
        <p class="mb-1 font-medium"><span class="capitalize" x-text="due"></span> assignments <span
                x-text="getClassName()"></span></p>
        <p class="text-sm text-gray-700"><span x-text="filteredAssignments.length"></span> assignment<span
                x-text="filteredAssignments.length == 1 ? '' : 's'"></span></p>
    </div>
    <div class="">
            ...
            <div>
                <template x-if="filteredAssignments.length > 0">
                    <div>
                        <div class="pt-2">
                            <template
                                x-for="assignment in filteredAssignments.filter(item => (new Date(item['due']).toDateString() == new Date().toDateString() && new Date(item['due']).getTime() >= new Date().getTime() ))">
                                <div>
                        ...
                        <div class="pt-4">
                            ...
                            <template
                                x-for="assignment in filteredAssignments.filter(assignment => (new Date(assignment['due']) > new Date(new Date().setHours(23,59,59,999)) && new Date(assignment['due']) <= new Date().setDate(new Date(new Date().setHours(23,59,59,999)).getDate() + 7) ) )">
                                <div>
                                    <x-assignments.assignment-card />
                                </div>
                            </template>
                            ...
                            <template
                                x-for="assignment in filteredAssignments.filter(assignment => (new Date(assignment['due']) > new Date().setDate(new Date(new Date().setHours(23,59,59,999)).getDate() + 7) ) )">
                                <div>
                                    <x-assignments.assignment-card />
                                </div>
                            </template>
                            ...
                            <template
                                x-for="assignment in filteredAssignments.filter(assignment => (new Date(assignment['due']).getTime() < new Date().getTime() || new Date(assignment['due']) < new Date() ))">
                                <div>
                                    <x-assignments.assignment-card />
                                </div>
                            </template>
        ...
        function assignmentList() {
            return {
                ...
                init: function() {
                    ...
                    this.$wire.getClassName(this.filterClass).then(result => {
                ...
                get filteredAssignments() {
                    ...
                    return this.assignments.filter(item =>
                        ...
                        (item['name'].toLowerCase().indexOf(this.query.toLowerCase()) > -1 || item['description']
                            .toLowerCase().indexOf(this.query.toLowerCase()) > -1)
                    ).sort((firstEl, secondEl) => new Date(secondEl['due']).getTime() - new Date(firstEl['due'])
                        .getTime());
                },
            }
        }
