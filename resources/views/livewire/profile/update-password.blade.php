...
x-data="{
  ...
  <x-ui.settings-card title="{{str_contains(Route::currentRouteName(), 'update') ? 'Update' : 'Set'}} password"
    ...
    <form wire:submit.prevent="save" style="max-width: 600px">
      <label class="w-full mt-8 mdc-text-field mdc-text-field--outlined mdc-text-field--with-trailing-icon" :class="{'mdc-text-field--invalid': errorMessages['password'] != undefined}" wire:ignore>
        <input class="mdc-text-field__input" aria-labelledby="password-label" :type="passwordField" autofill="new-password" wire:model.debounce="password" required />
      ...
      <p class="mt-4 text-gray-500">To keep your account secure, make sure that you do not use your new password for any of your other accounts, and that it is not something that can be easily guessed, like a family member or pet's name.</p>

      <label class="w-full mdc-text-field mdc-text-field--outlined mdc-text-field--with-trailing-icon mt-7" :class="{'mdc-text-field--invalid': errorMessages['passwordConfirmation'] != undefined}" wire:ignore>
