<div class="pt-20" x-data="schedules()">
        ...
        <div>
            <div class="pt-1 pb-6 border-b border-gray-200">
                <div class="h-10 my-2" wire:ignore>
                    <template x-if="editing != -1">
                        <div class="flex items-center h-full space-x-2">
                            <p class="flex-grow text-base">Currently editing <span class="font-bold"
                                    x-text="name"></span></p>
                ...
                <label class="mdc-text-field mdc-text-field--filled w-full mt-3 md:w-3/5" wire:ignore>
                    ...
                    <input class="mdc-text-field__input" type="text" aria-labelledby="schedule-name-label"
                ...
                <div class="flex mt-3 space-x-3">
                    <div class="w-full">
                        <x-ui.date-picker title="Start Date" bind="startDate" valid-date="beforeEnd" show-prev-years />
                        <x-ui.validation-error for="start_date" />
                    </div>

                    <div class="w-full">
                        <x-ui.date-picker title="End Date" bind="endDate" valid-date="afterStart" show-prev-years />
                        <x-ui.validation-error for="end_date" />
                    </div>
                </div>
                <div class="flex justify-end">
                    <button class="mdc-button mdc-button--raised mdc-button-ripple" type="button" x-bind="saveButton"
                        disabled wire:ignore>
                ...
                <template x-for="schedule in schedules">
                    <div class="flex px-4 py-2 space-x-5" :data-id="schedule.id">
                        <div class="flex-grow">
                            ...
                            <p class="text-sm text-gray-600"><span x-text="schedule.start"></span> to <span
                                    x-text="schedule.end"></span></p>
                        </div>
                        <div>
    ...
    <div class="mdc-dialog confirm-dialog delete-schedule-confirmation" id="confirm-dialog" wire:ignore>
        <div class="mdc-dialog__container">
            <div class="mdc-dialog__surface" role="alertdialog" aria-modal="true" aria-labelledby="my-dialog-title"
                ...
                <div class="mdc-dialog__actions">
                    <button class="mdc-button mdc-dialog__button cancel" data-mdc-dialog-action="cancel" type="button">
                        <div class="mdc-button__ripple"></div>
                        <span class="mdc-button__label">Cancel</span>
                    </button>
                    <button class="mdc-button mdc-dialog__button confirm" data-mdc-dialog-action="delete"
                        type="button">
                        <div class="mdc-button__ripple"></div>
                ...
                deleteButton: {
                    ['@click'](e) {
                        ...
                        scheduleDeleteDialog().then(() => {
                            this.$wire.delete(id).then(this.$wire.getSchedulesArray().then((
                                result) => {
                                this.schedules = result;
                            })).finally(
                                () => {
