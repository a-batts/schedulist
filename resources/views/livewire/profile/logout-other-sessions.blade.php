...
@hide-password-confirmation="passwordConfirmation = false">
  <x-ui.settings-card title="Your devices"
    ...
    <div style="max-width: 600px">
                    ...
                    <div class="flex items-center">
                        <div class="h-20 w-full">
                            ...
                            <div class="block">
                                ...
                                <div class="ml-11 text-gray-500">{{ $session->ip_address }}, {{ $session->agent->device() }} - {{$session->agent->browser()}}</div>
                            </div>
                            <div class="block sm:float-right sm:mr-1 sm:-mt-9">
                                @if ($session->is_current_device)
                                    <span class="material-icons check-icon-sm -va-5 mr-1 select-none">check_circle</span>
        ...
        @endif
        <div class="mt-5">
            <button class="mdc-button mdc-button--raised mdc-button-ripple tfa-button mt-5" x-on:click="fixBody(); passwordConfirmation = true; setTimeout(() => $refs.password.focus(), 250)" wire:ignore>
  ...
  <div class="modal-container fixed top-12 h-screen w-screen overflow-y-auto pb-6" x-transition x-show="passwordConfirmation" x-cloak wire:ignore.self>
    <div class="mdc-card mdc-card--outlined modal-card px-6 pt-6">
        <div>
            ...
            <div class="mt-3 text-left text-base text-gray-600">Before we can sign out your other devices, we need you to verify your identity.</div>
        </div>
        ...
        <div class="mt-5">
            <button class="mdc-button mdc-button-ripple mdc-button--raised float-right ml-4" wire:click="logoutOtherBrowserSessions()" wire:ignore>
                <span class="mdc-button__ripple"></span>Confirm
            </button>
            <button class="mdc-button mdc-button-ripple float-right" @click="passwordConfirmation = false" wire:ignore wire:loading.attr="disabled">
