  ...
  <x-ui.auth-card title="Verify that it's you" description="Please enter the six digit code displayed in your authenticator app (or a backup code) to continue.">
    <div x-data="{
      ...
      <form method="POST" action="/two-factor-challenge" id="twoFactorForm">
        @csrf
        <div class="h-20 pt-5 mt-8 mb-6" x-show="!recovery">
          <label class="w-full mdc-text-field mdc-text-field--outlined mdc-text-field--no-label @error('code')mdc-text-field--invalid @enderror" style="height: 70px;">
            ...
            <input id="code" class="text-4xl text-center mdc-text-field__input" type="tel" name="code" autofocus x-ref="code" 
        ...
        <div class="h-20 mt-8 mb-6" x-show="recovery" x-cloak>
          <p class="mb-4 -mt-4 text-sm">Recovery codes format: <span class="px-2 py-2 font-mono bg-gray-100 rounded-lg">XXXXXXXXXX-XXXXXXXXXX</span></p>
          <label class="w-full mdc-text-field mdc-text-field--outlined mdc-text-field--no-label">
            <span class="mdc-notched-outline">
              <span class="mdc-notched-outline__leading"></span>
              <span class="mdc-notched-outline__trailing"></span>
            </span>
