...
<div class="transition-width mdc-card mdc-card--outlined agenda-item absolute min-h-[3.75rem] w-full select-none transition-colors"
    ...
    <div class="mdc-card__primary-action h-full pb-2" tabindex="0"
        ...
        <p class="agenda-text-primary font-medium pointer-events-none"
            :class="{
                'text-lg ': view == 'day',
            ...
            x-text="item.name"></p>
        <p class="agenda-text-secondary text-sm pointer-events-none" x-text="item?.data?.['location'] ?? ''"></p>
        <p class="agenda-text-secondary text-sm transition-all pointer-events-none"
            :class="{ 'hidden': view == 'week' }">
